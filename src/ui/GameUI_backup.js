export class GameUI {
  ...
  setupEventListeners() {
    ...
    if (this.elements.centerRollDiceBtn) {
      this.elements.centerRollDiceBtn.addEventListener('click', () => {
        const dice = this.game.rollDice();
        this.updateDiceDisplay(dice[0], dice[1]);
      });
    }
    ...
    // Event listeners para botones centrales
    if (this.elements.centerBuyBtn) {
      this.elements.centerBuyBtn.addEventListener('click', () => {
        this.handleCenterBuy();
      });
    }

    if (this.elements.centerPassBtn) {
      this.elements.centerPassBtn.addEventListener('click', () => {
        this.handleCenterPass();
      });
    }

    // Event listener para carta de destino
    if (this.elements.destinyCardOk) {
      this.elements.destinyCardOk.addEventListener('click', () => {
        this.hideDestinyCard();
      });
    }
  }
  
  addLogMessage(message) {
    // Agregar mensaje a los logs tradicionales si existen
  ...
  addFloatingMessage(message) {
    ...
    this.elements.floatingMessages.appendChild(messageDiv);
    ...
    // Limitar número de mensajes flotantes
    const messages = this.elements.floatingMessages.children;
    if (messages.length > 5) {
      this.elements.floatingMessages.removeChild(messages[0]);
    }
  }
  
  showCenterButtons(canBuy, propertyName, propertyPrice) {
    if (!this.elements.centerActionButtons) return;
    
    // Actualizar botón de compra
    if (canBuy && this.elements.centerBuyBtn) {
      this.elements.centerBuyBtn.style.display = 'flex';
      const priceElement = this.elements.centerBuyBtn.querySelector('.btn-price');
      if (priceElement) {
        priceElement.textContent = `$${propertyPrice.toLocaleString()}`;
      }
      const textElement = this.elements.centerBuyBtn.querySelector('.btn-text');
      if (textElement) {
        textElement.textContent = `Comprar ${propertyName}`;
      }
    } else if (this.elements.centerBuyBtn) {
  ...
  showCenterDiceButton() {
    if (this.elements.centerDiceButton) {
      this.elements.centerDiceButton.style.display = 'flex';
    }
  }
  ...
  handleCenterBuy() {
    ...
    // Cambiar turno automáticamente después de 3 segundos
    this.addFloatingMessage('🏠 Propiedad comprada! Cambiando turno en 3 segundos...');
    setTimeout(() => {
      this.game.endTurn();
    }, 3000);
  }
  ...
  handleCenterPass() {
    ...
    // Cambiar turno inmediatamente
    this.addFloatingMessage('⏭️ Turno pasado');
    setTimeout(() => {
      this.game.endTurn();
    }, 500);
  }
  
  showDestinyCard(cardText) {
    if (!this.elements.destinyCardModal || !this.elements.destinyCardText) return;
    
    this.elements.destinyCardText.textContent = cardText;
    this.elements.destinyCardModal.style.display = 'flex';
    
    // Reiniciar animación de la carta
    const cardFlip = this.elements.destinyCardModal.querySelector('.card-flip');
    if (cardFlip) {
      cardFlip.style.animation = 'none';
      setTimeout(() => {
        cardFlip.style.animation = 'cardFlip 2s ease-in-out';
      }, 10);
    }
  }
  ...
  hideDestinyCard() {
    ...
    // Continuar con el juego después de cerrar la carta
  ...
  updateUI(gameState) {
    ...
    // Actualizar avatares de jugadores en esquinas
    this.updatePlayerAvatars(players, currentPlayer);
    ...
    // Actualizar estado de los botones del panel lateral (si existen)
    if (this.elements.rollDiceBtn) {
      this.elements.rollDiceBtn.disabled = !canRollDice;
    }
    if (this.elements.buyPropertyBtn) {
    ...
    // Actualizar texto de los botones del panel lateral
    if (currentPlayer && currentPlayer.isInJail && this.elements.rollDiceBtn) {
      this.elements.rollDiceBtn.textContent = 'Tirar para Salir de Cárcel';
    } else if (this.elements.rollDiceBtn) {
      this.elements.rollDiceBtn.textContent = 'Tirar Dados';
    }
    
    if (canBuyProperty && currentPlayer && this.elements.buyPropertyBtn) {
  ...
  updateBoardCenter(currentPlayer, canRollDice, canBuyProperty, canEndTurn, waitingForBuyDecision) {
    // Función deshabilitada - board-center fue eliminado
    // Los elementos currentTurnPlayer, turnStatus ya no existen
    return;
  ...
  startTurnTimer(phase = 'ROLL_DICE') {
    ...
    this.turnTimer = setInterval(() => {
      ...
      // Actualizar timer en avatar del jugador actual
      this.updateCurrentPlayerAvatar();
  ...
  updateCurrentPlayerAvatar() {
    // Actualizar solo el avatar del jugador actual con el timer
    const currentPlayerAvatar = document.querySelector('.player-avatar.current-turn');
    if (currentPlayerAvatar) {
      const avatarTimer = currentPlayerAvatar.querySelector('.avatar-timer');
      if (avatarTimer && this.currentTurnTime > 0) {
        avatarTimer.textContent = `${this.currentTurnTime}s`;
        
        // Aplicar clases de warning/danger según tiempo restante
        if (this.currentTurnTime <= 5) {
          avatarTimer.className = 'avatar-timer danger';
        } else if (this.currentTurnTime <= 10) {
          avatarTimer.className = 'avatar-timer warning';
        } else {
          avatarTimer.className = 'avatar-timer';
        }
      }
    }
  }
  ...
  updatePlayerAvatars(players, currentPlayer) {
    if (!players) return;
    ...
    players.forEach((player, index) => {
      ...
      const avatarCircle = avatar.querySelector('.avatar-circle');
      const avatarName = avatar.querySelector('.avatar-name');
      const avatarTimer = avatar.querySelector('.avatar-timer');
      
      if (avatarCircle && avatarName && avatarTimer) {
        // Establecer color del jugador
        avatarCircle.style.backgroundColor = player.color;
        avatarCircle.querySelector('.avatar-initial').textContent = player.name.charAt(0).toUpperCase();
        
        // Establecer nombre del jugador
        avatarName.textContent = player.name;
        
        // Mostrar/ocultar avatar según si el jugador está activo
        if (player.bankrupt) {
          avatar.style.opacity = '0.3';
          avatar.classList.remove('active', 'current-turn');
          avatarTimer.textContent = 'Bancarrota';
          avatarTimer.className = 'avatar-timer danger';
        } else {
          avatar.style.opacity = '1';
          avatar.classList.add('active');
          
          // Marcar si es el turno actual
          if (currentPlayer && player.name === currentPlayer.name) {
            avatar.classList.add('current-turn');
            
            // Mostrar timer si está activo
            if (this.turnTimer && this.currentTurnTime > 0) {
              avatarTimer.textContent = `${this.currentTurnTime}s`;
              ...
              avatarTimer.textContent = 'Tu turno';
              avatarTimer.className = 'avatar-timer';
            }
          } else {
            avatar.classList.remove('current-turn');
            avatarTimer.textContent = 'Esperando';
            avatarTimer.className = 'avatar-timer';
          }
        }
      }
    ...
    // Ocultar avatares no utilizados
    for (let i = players.length; i < 5; i++) {
      const avatar = document.getElementById(`player-avatar-${i}`);
      if (avatar) {
  ...
  updateAllPlayersInfo(players, currentPlayer) {
    ...
    sortedPlayers.forEach((player, index) => {
      ...
      if (player.bankrupt) {
        playerDiv.classList.add('bankrupt');
      }
      
      // Crear contenido del jugador
      const isCurrentPlayer = currentPlayer && player.name === currentPlayer.name;
      const propertiesCount = this.getPlayerPropertiesCount(player);
      
      playerDiv.innerHTML = `
        <div class="player-summary-header">
          <div class="player-summary-name" style="color: ${this.getPlayerColor(player.name)}">
            ${isCurrentPlayer ? '▶ ' : ''}${player.name}
            ${player.bankrupt ? ' 💀' : ''}
            ${player.isInJail ? ' 🔒' : ''}
          </div>
          <div class="player-summary-money ${player.money < 0 ? 'negative' : ''}">
            $${player.money.toLocaleString()}
          </div>
        </div>
        <div class="player-summary-properties">
          🏠 ${propertiesCount} propiedades | Pos: ${player.position}
        </div>
      `;
  ...
  getPlayerPropertiesCount(player) {
    // Contar propiedades del jugador
    return (player.properties ? player.properties.length : 0) + 
           (player.railroads ? player.railroads.length : 0) + 
           (player.utilities ? player.utilities.length : 0);
  }
